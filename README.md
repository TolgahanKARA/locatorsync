...
- **Robot Güncelle:** Ayrı sekme — Vue'da hem `data-test` hem `id` olan **statik** elementleri tespit eder (dinamik `:data-test="expr"` hariç — runtime değeri bilinmiyor); Robot dosyalarındaki `css=[data-test='X']` locatorlarını `id=X` ile değiştirir. Önizle → satır satır seçim → Dry-Run → Seçilileri Uygula akışı. Kod: `core/patcher/RobotLocatorUpdater.py`. Eşleşme bulunamazsa debug kutusu: kullanılan Vue yolu + "Proje Ayarlarını Düzenle" butonu gösterilir; `vue_elements_with_id=0` ise yol yanlış veya ID Ekle henüz çalıştırılmamış demektir.
