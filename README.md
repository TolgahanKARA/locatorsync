...
- **Robot Güncelle:** Ayrı sekme — Vue'da hem `data-test` hem `id` olan elementleri tespit eder; Robot dosyalarındaki `css=[data-test='X']` locatorlarını `id=X` ile değiştirir. Önizle → satır satır seçim → Dry-Run → Seçilileri Uygula akışı. Kod: `core/patcher/RobotLocatorUpdater.py`
